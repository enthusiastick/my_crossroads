...
def create
  source_character = character = Character.find_by(slug: params[:character_id])
  ingredient = Ingredient.find_by_id(params["ingredient_transfer"]["ingredient"].to_i)
  source_inventory = Inventory.where("character_id = ? AND ingredient_id = ?", source_character.id, ingredient.id).first
  transfer_quantity = params["ingredient_transfer"][:quantity].to_i
  target_character = character = Character.find_by(slug:params["ingredient_transfer"][:target_inventory])
  target_inventory = Inventory.where("character_id = ? AND ingredient_id = ?", target_character.id, ingredient.id).first
  ...
  if transfer_quantity <= source_inventory.quantity && target_inventory && transfer_quantity > 0
    ...
    target_inventory = Inventory.create(character:target_character, ingredient:ingredient, quantity:transfer_quantity)
    source_inventory.update(quantity:source_total)
    ingredient_transfer = IngredientTransfer.new(
      source_inventory:source_inventory,
      target_inventory:target_inventory,
